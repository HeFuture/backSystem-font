...
const getFirstPageList = async () => {
    const res = await getAdminListData(1, '消息管理员')
    tableData.value = res.data
}
getFirstPageList()
...
const currentChange = async (value: number) => {
    ...
    const res = await getAdminListData(value, '消息管理员')
    tableData.value = res.data
}
...
bus.on('adminDialogOff', async (id: number) => {
    ...
    // 创建管理员
    if (id == 1) {
        const res = await getAdminListData(pageinationData.currentpage, '消息管理员')
        tableData.value = res.data
    }
    // 编辑管理员
    else if (id == 2) {
