<template>
    ...
    <div class="table-wrapped">
        ...
        <div class="table-fotter">
            <el-pagination :page-size="1" v-model:current-page="pageinationData.currentpage" layout="prev, pager, next"
                :page-count="pageinationData.pageCont" @current-change="currentChange" :total="adminTotal" />
        </div>
    </div>
...
const searchAdmin = async () => {
    ...
    if (adminAccount.value == null || adminAccount.value.trim() === '') {
        ...
        const res = await getAdminListData(pageinationData.currentpage, '产品管理员')
...
const getFirstPageList = async () => {
    const res = await getAdminListData(1, '产品管理员')
    tableData.value = res.data
}
getFirstPageList()
...
const currentChange = async (value: number) => {
    ...
    const res = await getAdminListData(value, '产品管理员')
    tableData.value = res.data
}
...
bus.on('adminDialogOff', async (id: number) => {
    ...
    // 对管理员降职
        ...
        const res = await getAdminListData(pageinationData.currentpage, '产品管理员')
        tableData.value = res.data
        // if (res.data.length == 0 && pageinationData.currentpage > 1) {
        //     pageinationData.currentpage = current - 1
        //     const res = await getAdminListData(pageinationData.currentpage, '产品管理员')
        //     if (res.status == 200) {
        //         tableData.value = res.data
        //     }
